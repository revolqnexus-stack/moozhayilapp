...
| **Open follow-ups** | client matrix gaps; in-flight pause; Aura chat rate timestamp; live-device pass |
...
| **Follow-ups** | live-device pass; ops sign-off on lock duration |

### Fix 5: Enrollment → first payment handoff — **closed**

| Item | Detail |
|------|--------|
| **Status** | Closed on `fix/closure-fix3-fix2` — **not merged to main** |
| **Approach** | Option B (transcript): keep Contribute separate; auto-handoff after enroll for Crest/Dhanam |
| **Flow** | `confirmation_screen` creates goal → routes to `/goals/:id/contribute?firstPayment=1&amountPaise=…` with locked amount; Step 2 of 2 header; Pay now → My Plans on success |
| **Copy** | Payment setup + plan highlights aligned (“locked on payment” not “at enrollment”) |
| **Tests** | `enrollment_handoff_test.dart` (2) |
| **Follow-ups** | Live-device Crest/Dhanam E2E; legal sign-off on booking-advance timing |

### Production readiness (2026-09-21)

| Item | Status |
|------|--------|
| Top 5 fixes (1–5) | **All closed** on branch |
| Flutter tests | **119/119 pass** (harness repaired) |
| API quote/KYC tests | 16/16 pass (orders webhook sig test pre-existing) |
| Merge to main | Pending your approval |
| Infra | Render deploy + live Razorpay/MSG91/KYC/Firebase per `DEPLOYMENT_CHECKLIST.md` |
